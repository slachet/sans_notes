...
API Gateway functions as a proxy, and the target cannot know the source IP address.
...
- `-m` is hash mode specifier. `-m 0` is md5.
...
Wordlist (permutation) for possible bucket name is the key for successful discovery.
...
1. Convert password into upper-case
2. Pad `0x00` to 14 bytes as total
3. Split 14 bytes into 2 of 7 bytes
4. Encrytp `KGS!@#$%` with the 7 bytes for each
5. Attackers only have to crack this 7 bytes

#### NT Hashes (name of hash function)

- Modern
- A bit weak
- Hash gets encrypted
- More than 14 characters password enfoces use of NT hash
...
- PBKDF2 (Password-Based Key Derivation Function): Variable number of hash calcuration (e.g., WPA's psk, and WPA2's psk)
- Scrypt: requires 1000x as much memroy (not focusing on calculation)
- Yescript: Large lookup table required
- Argon2: Unproven algorithm
